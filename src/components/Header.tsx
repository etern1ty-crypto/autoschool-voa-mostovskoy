...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-[#080A0F]/95 backdrop-blur-xl border-b border-white/10 shadow-lg">
      {/* Top Utility Bar */}
      <div className="bg-[#0c0f0f] border-b border-white/5 text-xs text-slate-300 py-1.5 px-4">
        <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-3 text-xs">
            <span className="text-national-red font-bold">ВОА РФ</span>
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-2.5 py-0.5 rounded border text-xs font-bold transition ${
                isActive
            ...
            >
              <Eye className="w-3.5 h-3.5 keep-icon" />
              <span className="hidden sm:inline">Версия для слабовидящих</span>
              <span className="sm:hidden">ГОСТ</span>
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between relative">
        ...
        <a href="#hero" className="flex items-center gap-3 group relative z-10">
          <div className="w-13 h-13 rounded-full bg-[#12161F] border-2 border-national-red shadow-[0_0_18px_rgba(227,30,36,0.45)] flex items-center justify-center p-1 group-hover:scale-105 transition-transform overflow-hidden">
            <img
          ...
          <div className="text-left">
            <div className="font-extrabold text-base tracking-wider text-white flex items-center gap-1.5 uppercase">
        ...
        <div className="lg:hidden bg-[#0c0f0f] border-b border-white/10 px-4 py-6 space-y-4 animate-fadeIn">
          <nav className="flex flex-col gap-3 text-sm font-bold uppercase tracking-wider">
            ...
            <button
              ...
              className="text-left px-3 py-2 text-national-red bg-national-red/10 rounded"
            >
              Сведения об образовательной организации
            </button>
          </nav>
          <div className="pt-4 border-t border-white/10 flex flex-col gap-3">
            <a href={`tel:${SCHOOL_INFO.phoneClean}`} className="flex items-center gap-2 text-white font-bold p-3 bg-surface-card rounded border border-white/10">
